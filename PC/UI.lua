				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Jerry Hub")
							if not fold then
								makefolder("./Jerry Hub")
							end
							fold = isfolder("./Jerry Hub/Themes")
							if not fold then
								makefolder("./Jerry Hub/Themes")
							end
							fold = isfolder("./Jerry Hub/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./Jerry Hub Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Jerry Hub/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Jerry Hub/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Jerry Hub/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Jerry Hub") then
						makefolder("./Jerry Hub")
					end
					local common_string = "./Jerry Hub/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Jerry Hub/" .. tostring(custom_workspace or library.WorkspaceName)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		for _, v in next, {{"Main", "main"},{"Element Text", "elementText"}} do
